...
Accordingly, our response variable is `switch` - a binary value that indicates whether the user of an unsafe well in an area of Araihazar Upazila, Bangladesh switched to a different well. If `switch` takes a value of 1, it indicates that the household switched to a new well; conversely, if `switch` takes a value of 0, the household continued using the current well. The remaining 56 variables are all predictor variables, which contain information about the household and wells in the area. For the purpose of this analysis, we only considered 7 relevant predictors from the available 56 and recoded some of them to remove some inconsistencies and redundancies, while improving interpretability.


Only two of the relevant predictors are continuous:

- `dist` indicates the distance (in meters) to the closest known safe well.

- `arsenic` indicates the arsenic concentration (in micrograms per Liter) of a household's well.

The remaining predictors used are discrete:

- `educ` indicates the education level of the head of the household in years of education completed.

- `educ4` indicates the education level of the head of the household as a categorical variable with 4 levels: 0, 1-8, 9-12, or 12+ years of education completed.

- `assoc` indicates if any member of the household is active in community organizations. Originally the survey options had more levels to specify the type of organization ([0] no, [1] informal savings/micro-credit, [2] bazaar samiti, [3] other), but the discrepancy between no association and any association was more significant.

- `use` indicates how the household used the well. The survey asked if they used the well for drinking or cooking, but "sometimes" was also an option. Thus, it seemed logical to group the responses into 5 levels: Unfrequently, Drinking, Cooking, or Both.

- `status_perceived` indicates how the user perceived the status or condition of the well (Unsafe, Safe, or Don't Know). 

- `well_shift` indicates if the household had shifted to a different well in the past 3 years before the survey was conducted.
...
################# SECOND STEP
no_educ_interac <- stan_glm(switch ~ dist100 + as100 + assoc + use + status_perceived + well_shift, data = wells,
                 family = binomial(link = "logit"), 
                 prior = t, prior_intercept = t,
                 seed = seed,
                 refresh = 0)
