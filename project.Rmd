...
rm_educ_loo <- loo_compare(loo_main1, loo_no_educ1, loo_no_arsenic, loo_no_assoc, loo_no_use, loo_no_status, loo_no_shift)
...
rm_first_step <- loo_compare(loo_no_educ1, loo_no_educ_dist, loo_no_educ_as, loo_no_educ_assoc, loo_no_educ_use, loo_no_educ_status, loo_no_educ_shift)
...
rm_second_step <- loo_compare(loo_no_educ_interac, loo_no_educ_interac_noassta, loo_no_educ_interac_noasdist, loo_no_educ_interac_nodistshift)
...
no_educ_interac_accuracy <- round(mean(xor(ploo>0.5,as.integer(wells$switch==0))),3)
...
no_educ_interac_noassta_accuracy <- round(mean(xor(ploo>0.5,as.integer(wells$switch==0))),3)
...
no_educ_interac_noasdist_accuracy <- round(mean(xor(ploo>0.5,as.integer(wells$switch==0))),3)
...
no_educ_interac_nodistshift_accuracy <- round(mean(xor(ploo>0.5,as.integer(wells$switch==0))),3)
...
data.frame(Coefficient = round(coef(no_educ_interac), 3),
           CI = round(posterior_interval(no_educ_interac, prob = 0.95), 3)) %>%
  kable()
```
...
Here we have displayed the output from the `loo_compare()` function at each stage of the model selection scheme.

\vspace{10mm}

```{r loo, echo=FALSE, fig.height=4, fig.width=6.5}
rm_educ_loo
rm_first_step
rm_second_step
```