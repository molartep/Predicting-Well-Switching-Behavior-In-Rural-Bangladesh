...
wells.data <- cbind (switch, arsenic, dist, assoc, educ, educ4, use, 
                     status_perceived, well_change, well_shift)
...
hist(dist, breaks=seq(0,10+max(dist[!is.na(dist)]),10), 
     xlab="Distance (in meters) to nearest safe well", ylab="Frequency", 
     main="Distance to Wells")
# Arsenic Histogram
hist(arsenic, breaks=seq(0,1+max(arsenic[!is.na(arsenic)]),0.5), 
     xlab="Arsenic content (UNITS)", ylab="Frequency", main="Arsenic Content")
# Education Histogram
...
barplot(prop.table(table(status_perceived)), main = "Perceived Status", 
        ...
        ylim = c(0,0.9))
# Change
barplot(prop.table(table(well_change)), main = "Well Change (Last 3 Years)", 
        xlab = "0 = No,  1 = Yes", ylab = "Proportion", ylim = c(0,0.6))
# Shift
...
### Response Variable
barplot(prop.table(table(switch)), main = "Well Shift (Last 3 Years)", 
        xlab = "0 = No,  1 = Yes", ylab = "Proportion", ylim = c(0,0.6))
```
